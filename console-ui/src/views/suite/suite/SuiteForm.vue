...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        suiteCode: [
          { required: true, message: this.$t('suite.inputSuiteCode'), trigger: 'blur' },
          { pattern: /^[a-zA-Z0-9_]+$/, message: this.$t('suite.suiteCodeFormat'), trigger: 'blur' },
        ],
        suiteName: [{ required: true, message: this.$t('suite.inputSuiteName'), trigger: 'blur' }],
      },
    };
  },
  computed: {
    ...
    title() {
      if (this.editItem) {
        return this.$t('suite.editSuite');
      }
      return this.$t('suite.addSuite');
    },
  },
  methods: {
    onCancel() {
    ...
    beforeSuiteImageUpload(file) {
      ...
      if (!isJPG && !isPNG) {
        ElMessage.error(this.$t('suite.imageFormatError'));
        return false;
      }
      if (!isLt30K) {
        ElMessage.error(this.$t('suite.imageSizeError'));
        return false;
      }
  ...
  <el-dialog v-model="dialogVisible" :title="title" width="600">
    <div class="form">
      <el-form ref="formRef" label-position="top" :model="formValue" :rules="rules">
        <el-form-item :label="$t('suite.suiteImage')">
        ...
        <el-form-item :label="$t('suite.suiteCode')" prop="suiteCode">
          <el-input v-model="formValue.suiteCode" maxlength="20" :disabled="isEditMode"/>
        </el-form-item>
        <el-form-item :label="$t('suite.suiteName')" prop="suiteName">
          <el-input v-model="formValue.suiteName" maxlength="30" />
        </el-form-item>
        <el-form-item :label="$t('suite.suiteDesc')" prop="suiteDesc">
