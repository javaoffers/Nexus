...
import { DrawNodeHelper } from "./DrawNodeHelper";
...
export class VerticalLayout {
  ...
  drawLines() {
    ...
    const flowLines = this.lineContainer.selectAll('.flow-line').data(lines, (d) => `${d.from}-${d.to}`);
  ...
  getPolyline(fromNode, toNode) {
    const from = fromNode.getContentBoxCenter();
    const to = toNode.getContentBoxCenter();
    // 最后一个分支节点
