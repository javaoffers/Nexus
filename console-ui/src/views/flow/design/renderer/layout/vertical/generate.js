...
function layoutNormal(prev, node) {
  let left = prev.left;
...
function layoutCondition(prev, node) {
...
function alignItemCenter(node) {
  node.setRelative(node.left - node.width / 2, node.top);
}

function offsetBranch(node) {
  node.setRelative(node.left + node.width / 2, node.top);
}

export function setLayoutToMap(node, dataNodeMap) {
