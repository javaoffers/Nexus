import { ElementType } from "@/views/flow/design";
...
export class DrawNodeHelper {
     ...
     * 画节点
     */
    drawNode(container, node, type = 'update') {
    ...
    drawNormal(container, node, type) {
    ...
    drawCondition(container, node, type) {
        if (type === 'enter') {
    ...
    drawBranch(container, branch, type) {
    ...
    drawAddIcon(container, node, type) {
        ...
        if (type === 'enter') {
            ...
            addButton.append('circle').attr('cx', 0).attr('cy', 0).attr('r', btn_radius).attr('fill', '#5B5FE6');
    ...
    drawHoverButtons(container, node) {
        ...
        btns.forEach((btn, i) => {
            ...
            flowBtns.append('circle').attr('cx', 0).attr('cy', 0).attr('r', '15').attr('fill', '#fff').attr('stroke', '#86909C');

            flowBtns.append('use').attr('href', `#icon-${btn}`).attr('width', 16).attr('height', 16).attr('x', -8).attr('y', -8).attr('fill', '#86909C');
        });
    }

    drawNormalCard(flowNode, node, width, height) {
        const { data } = node;
    ...
    drawStartNode(container, node, type){
        container.append('g')
            .attr('transform', `translate(8, -2)`)
            .append('use').attr('href', `#icon-start`).attr('width', 50).attr('height', 50).attr('x', -8).attr('y', -8).attr('fill', '#00B42A');
        this.drawAddIcon(container, node, type);
    }

    drawEndNode(container){
        container.append('g')
            .attr('transform', `translate(6, -2)`)
            .append('use').attr('href', `#icon-end`).attr('width', 57).attr('height', 57).attr('x', -8).attr('y', -8).attr('fill', '#86909C');
    }
}
